...
function calcTax($amount,$preferencecompany) {
	if($amount > $preferencecompany['tax_minimum_amount']){
		$tax_total	= $amount * $preferencecompany['tax_percentage'] / 100;
	}else{
		$tax_total 	= 0;
	}
	return nominal($tax_total);
}
?>	
								...
								foreach ($acctdepositoprofitsharingcheck as $key=>$val){
									...
									echo"
										<tr>			
											...
											<td style='text-align:right;background-color:".$warna."'>".calcTax($val['deposito_profit_sharing_amount'],$preferencecompany)."</td>";
