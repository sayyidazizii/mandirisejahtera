...
<script>
	...
	$(document).ready(function(){
        $("#Save").click(function(e){
			...
			var angsuran_total 			= document.getElementById('angsuran_total').val;
			...
			$.ajax({
					type: 'post',
					data: dataString,       
					url: '<?php echo base_url();?>cash-payments/process-add',
					success: function (results) {
						
						idlastpayment = results;
						if(idlastpayment != 'error'){
							openInNewTab(base_url+"cash-payments/print-note/"+idlastpayment);
						}
						document.location = base_url+"cash-payments/add";
					}
			});
		});
    });
...
<?php
	...
	// print_r($credits_payment_day_of_delay);exit;
...
<?php
// print_r($data); acctcashpayment_last
							...
							<div class="col-md-5">
								<table width="100%">
									<tr>
									 ...
									 <tr>
										<td width="35%">Sanksi Bulan/Minggu Ini</td>
										<td width="5%">:</td> 
										<td width="60%">
									...
									<tr>
									 ...
									 <tr>
										<td>Saldo Cicilan<span class="required">*</span></td>
										<td></td>
									 ...
									 <tr>
										...
										<td>:</td> 
										<td>
											<?php if($accountcredit['payment_type_id'] == 4 || $accountcredit['payment_type_id'] == 3  || $accountcredit['payment_type_id'] == 2 ){ ?>
									...
									<tr>
									 ...
									 <tr>
										...
										<td>:</td> 
										<td>
											<input type="text" class="easyui-textbox" name="angsuran_total_view1" id="angsuran_total_view1" value="<?php echo number_format($accountcredit['credits_account_payment_amount'], 2); ?>" readonly/>
