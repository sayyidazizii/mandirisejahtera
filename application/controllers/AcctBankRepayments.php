<?php
	...
	Class AcctBankRepayments extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('MainPage_model');
			$this->load->model('AcctBankRepayment_model');
		...
		public function index(){
			...
			$data['main_view']['content']		= 'AcctBankRepayment/ListAcctBankRepayment_view';
			$this->load->view('MainPage_view', $data);
		}

		public function filterAcctBankRepayment(){
			$data = array (
			...
			$this->session->set_userdata('filter-AcctBankRepayment', $data);
			redirect('AcctBankRepayments');
		}
		public function reset(){
			$this->session->unset_userdata('filter-AcctBankRepayment');
			redirect('AcctBankRepayments');
		}

		public function getAcctBankRepayment(){
			$auth 	= $this->session->userdata('auth');
			$sesi	= $this->session->userdata('filter-AcctBankRepayment');
			if(!is_array($sesi)){
			...
			$list = $this->AcctBankRepayment_model->get_datatables($sesi['start_date'] , $sesi['end_date'], $sesi['credits_id'], $sesi['branch_id']);
	        $data = array();
	        $no = $_POST['start'];
	        foreach ($list as $cashpayment) {
			    ...
			    $row[] = '<a href="'.base_url().'AcctBankRepayments/printNoteBankPayment/'.$cashpayment->credits_payment_id.'" class="btn btn-xs blue" role="button"><i class="fa fa-print"></i> Kwitansi</a>';
	            $data[] = $row;
	        }
	        ...
	        $output = array(
	                        "draw" 				=> $_POST['draw'],
	                        "recordsTotal" 		=> $this->AcctBankRepayment_model->count_all($sesi['start_date'] , $sesi['end_date'], $sesi['credits_id'], $sesi['branch_id']),
	                        "recordsFiltered" 	=> $this->AcctBankRepayment_model->count_filtered($sesi['start_date'] , $sesi['end_date'], $sesi['credits_id'], $sesi['branch_id']),
	                        "data" 				=> $data,
	                );
		...
		public function addAcctBankRepayment(){	
			...
			$data['main_view']['acctbankaccount']			= create_double($this->AcctBankRepayment_model->getAcctBankAccount(),'bank_account_id', 'bank_account_code');	
			
			$data['main_view']['accountcredit']			= $this->AcctCreditAccount_model->getDetailByID($credits_account_id);
			$data['main_view']['detailpayment']			= $this->AcctBankRepayment_model->getDataByIDCredit($credits_account_id);
			$data['main_view']['content']				= 'AcctBankRepayment/FormAddAcctBankRepayment_view';
			$this->load->view('MainPage_view',$data);
		}
		...
		public function getDetailPayment(){
			...
			$data['detailpayment']				= $this->AcctBankRepayment_model->getDataByIDCredit($credits_account_id);
			$data['content']					= 'AcctBankRepayment/ListPaymentByCreditAccount_view';
			$this->load->view('AcctBankRepayment/ListPaymentByCreditAccount_view',$data);
		}
		...
		public function akadlisttunai(){
	        ...
	        foreach ($list as $customers) {
	            ...
	            $row[] = tgltoview($customers->credits_account_due_date);
	             $row[] = '<a href="'.base_url().'AcctBankRepayments/addAcctBankRepayment/'.$customers->credits_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
		...
		public function processAddAcctBankRepayment(){
			$auth 				= $this->session->userdata('auth');
			...
			$credits_payment_token 							= $this->AcctBankRepayment_model->getCreditsPaymentToken($data['credits_payment_token']);


			if($this->form_validation->run()==true){
				if($credits_payment_token->num_rows() == 0){
					if($this->AcctBankRepayment_model->insert($data)){
						$updatedata = array(
						...
						if($member_mandatory_savings <> 0 || $member_mandatory_savings <> ''){
							...
							$this->AcctBankRepayment_model->insertAcctSavingsMemberDetail($data_detail);
						}

						$AcctBankRepayment_last 							= $this->AcctBankRepayment_model->AcctBankRepaymentLast($data['created_id']);
						
						$data_journal = array(
							'branch_id'						=> $auth['branch_id'],
							'journal_voucher_period' 		=> $journal_voucher_period,
							'journal_voucher_date'			=> date('Y-m-d'),
							'journal_voucher_title'			=> 'ANGSURAN BANK '.$AcctBankRepayment_last['credits_name'].' '.$AcctBankRepayment_last['member_name'],
							'journal_voucher_description'	=> 'ANGSURAN BANK '.$AcctBankRepayment_last['credits_name'].' '.$AcctBankRepayment_last['member_name'],
							'journal_voucher_token'			=> $data['credits_payment_token'],
							'transaction_module_id'			=> $transaction_module_id,
							'transaction_module_code'		=> $transaction_module_code,
							'transaction_journal_id' 		=> $AcctBankRepayment_last['credits_payment_id'],
							'transaction_journal_no' 		=> $AcctBankRepayment_last['credits_account_serial'],
						...
						if(!empty($member_mandatory_savings) || $member_mandatory_savings > 0 || $member_mandatory_savings != ''){
							...
							$data_credit =array(
								...
								'journal_voucher_description'	=> 'SETORAN BANK '.$AcctBankRepayment_last['member_name'],
								'journal_voucher_amount'		=> $member_mandatory_savings,
						...
						$this->session->unset_userdata('addAcctBankRepayment-'.$sesi['unique']);
						$this->session->unset_userdata('acctcreditspaymentcashtoken-'.$sesi['unique']);
						$this->session->set_userdata('message',$msg);
						redirect('AcctBankRepayments/printNoteBankPayment/'.$AcctBankRepayment_last['credits_payment_id']);
					}else{
						$this->session->set_userdata('addAcctBankRepayment-'.$unique['unique'],$data);
						$msg = "<div class='alert alert-danger alert-dismissable'>
								<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
									Tambah Data Pembayaran Pinjaman Tidak Berhasil
								</div> ";
						$this->session->set_userdata('message',$msg);
						redirect('AcctBankRepayments/addAcctBankRepayment');
					}
				} else {
					$AcctBankRepayment_last 				= $this->AcctBankRepayment_model->AcctBankRepaymentLast($data['created_id']);
		...
		public function printNoteBankPayment(){
			...
			$preferencecompany 		= $this->AcctCreditAccount_model->getPreferenceCompany();
			$acctcreditspayment	 	= $this->AcctBankRepayment_model->getAcctCreditspayment_Detail($credits_payment_id);
		...
		public function indCashLessPayment(){
			...
			$data['main_view']['acctcredits']	= create_double($this->AcctCreditAccount_model->getAcctCredits(),'credits_id', 'credits_name');
			$data['main_view']['corebranch']	= create_double($this->AcctCreditAccount_model->getCoreBranch(),'branch_id', 'branch_name');
			$data['main_view']['content']		= 'AcctBankRepayment/ListAcctNonCashPayment_view';
			$this->load->view('MainPage_view', $data);
		}
		...
		public function filteracctcashlesspayment(){
			...
			$this->session->set_userdata('filter-AcctBankRepaymentless', $data);
			redirect('AcctBankRepayments/indCashLessPayment');
		}

		public function reset_cashless(){
			$this->session->unset_userdata('filter-AcctBankRepaymentless');
			redirect('AcctBankRepayments/indCashLessPayment');
		}

		public function getAcctCashLessPayment(){
			$auth 	= $this->session->userdata('auth');
			$sesi	= $this->session->userdata('filter-AcctBankRepaymentless');
			if(!is_array($sesi)){
				$sesi['start_date']		= date('Y-m-d');
				$sesi['end_date']		= date('Y-m-d');
				$sesi['credits_id']		='';
	        ...
	        foreach ($list as $cashpayment) {
	            ...
	            if($cashpayment->credits_payment_type == 1){
		            ...
		            $row[] = $cashpayment->member_name;
		            $row[] = $cashpayment->credits_name;
		            $row[] = $this->AcctBankRepayment_model->getSavingsAccountNO($cashpayment->savings_account_id);
		            $row[] = tgltoview($cashpayment->credits_payment_date);
		            $row[] = number_format($cashpayment->credits_payment_principal, 2);
		            $row[] = number_format($cashpayment->credits_payment_interest, 2);
				    $row[] = '<a href="'.base_url().'AcctBankRepayments/printNoteCashLessPayment/'.$cashpayment->credits_payment_id.'" class="btn btn-xs blue" role="button"><i class="fa fa-print"></i> Kwitansi</a>';
		            $data[] = $row;
	            }
		...
		public function addCashlessPayment(){	
			...
			$data['main_view']['content']				= 'AcctBankRepayment/FormAddCashPaymentNon_view';
			$this->load->view('MainPage_view',$data);
		}
        ...
        foreach ($list as $customers) {
			...
			$row[] = '<a href="'.base_url().'AcctBankRepayments/addCashlessPayment/'.$segment3.'/'.$customers->savings_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
		...
		public function akadlist(){
	        ...
	        foreach ($list as $customers) {
	            ...
	            $row[] = tgltoview($customers->credits_account_date);
	            $row[] = tgltoview($customers->credits_account_due_date);
	             $row[] = '<a href="'.base_url().'AcctBankRepayments/addCashlessPayment/'.$customers->credits_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
		...
		public function AcctBankRepaymentsProcess(){
			$auth 			= $this->session->userdata('auth');
			...
			if($simpanan['savings_account_last_balance'] < $total){
				...
				$this->session->set_userdata('message',$msg);
				redirect('AcctBankRepayments/addCashlessPayment');
			}
			...
			if($this->form_validation->run()==true){
				if($this->AcctBankRepayment_model->insert($data_cash)){
					$updatedata = array(
					...
					$AcctBankRepayment_last 	= $this->AcctBankRepayment_model->AcctBankRepaymentLast($data_cash['created_id']);
					...
					$data_journal = array(
						...
						'journal_voucher_period' 		=> $journal_voucher_period,
						'journal_voucher_date'			=> date('Y-m-d'),
						'journal_voucher_title'			=> 'ANGSURAN NON BANK '.$AcctBankRepayment_last['credits_name'].' '.$AcctBankRepayment_last['member_name'],
						'journal_voucher_description'	=> 'ANGSURAN NON BANK '.$AcctBankRepayment_last['credits_name'].' '.$AcctBankRepayment_last['member_name'],
						'transaction_module_id'			=> $transaction_module_id,
						'transaction_module_code'		=> $transaction_module_code,
						'transaction_journal_id' 		=> $AcctBankRepayment_last['credits_payment_id'],
						'transaction_journal_no' 		=> $AcctBankRepayment_last['credits_account_serial'],
					...
					// print_r($AcctBankRepayment_last);exit;
					
					$this->AcctCreditAccount_model->insertAcctJournalVoucher($data_journal);

					$journal_voucher_id 		= $this->AcctCreditAccount_model->getJournalVoucherID($data_cash['created_id']);

					$savingsaccount_id 			= $this->AcctBankRepayment_model->getSavingsAccountID($mutasi_data['savings_id']);
					...
					$receivable_account_id 		= $this->AcctCreditAccount_model->getReceivableAccountID($AcctBankRepayment_last['credits_id']);
					...
					$this->session->set_userdata('addAcctBankRepayment-'.$unique['unique'],$data_cash);
					$msg = "<div class='alert alert-danger alert-dismissable'>
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
								Tambah Data Pembayaran Pinjaman Tidak Berhasil
							</div> ";
		...
		public function printNoteCashLessPayment(){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    ...
			    <tr>
			        ...
			        <td width=\"50%\"><div style=\"text-align: left;\">: ".$this->AcctBankRepayment_model->getSavingsAccountNO($acctcreditspayment['savings_account_id'])."</div></td>
			    </tr>
			    <tr>
			        <td width=\"20%\"><div style=\"text-align: left;\">Alamat</div></td>
			        <td width=\"50%\"><div style=\"text-align: left;\">: ".$acctcreditspayment['member_address']."</div></td>
		...
		public function reset_data(){
			$unique 	= $this->session->userdata('unique');
			$sessions	= $this->session->unset_userdata('addAcctBankRepayment-'.$unique['unique']);
			redirect('AcctBankRepayments/processAddAcctBankRepayment');
		}
		...
		public function historyPayment(){
			...
			$data['main_view']['content']					= 'AcctBankRepayment/ListHistoryAcctCreditsPayment_view';
			$this->load->view('MainPage_view',$data);
		}
		...
		public function creditList(){
	        ...
	        foreach ($list as $customers) {
	            ...
	            $row[] = $customers->member_name;
	            $row[] = $customers->member_no;
	             $row[] = '<a href="'.base_url().'AcctBankRepayments/historyPayment/'.$customers->credits_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
