			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			foreach ($acctbalancesheetreport_left as $keyLeft => $valLeft) {
									...
									if($valLeft['report_type1']	== 10){
										$last_balance10 	= 0;
										$shutahunberjalan 	= $this->AcctBalanceSheetReportNew1_model->getSHUTahunBerjalan($valLeft['account_id2'], $sesi['branch_id'], $month, $year);
										foreach($shutahunberjalan as $keyshu => $valshu){
											$last_balance10 = $last_balance10 + $valshu['mutation_in_amount'] - $valshu['mutation_out_amount'];
										}
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			...
			        			foreach ($acctbalancesheetreport_right as $keyRight => $valRight) {
									...
									if($valRight['report_type2']	== 10){
										$last_balance210	= 0;
										$shutahunberjalan 	= $this->AcctBalanceSheetReportNew1_model->getSHUTahunBerjalan($valRight['account_id2'], $sesi['branch_id'], $month, $year);
										foreach($shutahunberjalan as $keyshu => $valshu){
					...
					if(is_numeric($keyRight)){
						...
						if($valRight['report_type2'] == 11){
							if(!empty($valRight['report_formula2']) && !empty($valRight['report_operator2'])){
								...
								$this->excel->getActiveSheet()->setCellValue('D'.$j, $report_tab1.$valRight['account_name2']);
								$this->excel->getActiveSheet()->setCellValue('E'.$j, $report_tab1.$total_account_amount210);
						...
						if($valRight['report_type2'] == 5){
							if(!empty($valRight['report_formula2']) && !empty($valRight['report_operator2'])){
								...
								$this->excel->getActiveSheet()->setCellValue('D'.$j, $report_tab2.$valRight['account_name2']);
								$this->excel->getActiveSheet()->setCellValue('E'.$j, $report_tab2.$total_account_amount2+$total_account_amount210);
